...
<script>
  ...
  export default {
    ...
    methods: {
      searchResources () {
        let vm = this
        getApi('searchResources', function (res) {
          if (res.data.code === 200) {
            for (let i = 0; i < res.data.data.length; i++) {
              res.data.data[i].selectList = []
            }
        ...
        this.$refs.addForm.validate((valid) => {
          if (valid) {
            this.$confirm('确认提交吗？', '提示', {}).then(() => {
              ...
              let vm = this
              getApi('addResources', addResourcesParams, function (res) {
      ...
      handleDel (index, row) {
        this.$confirm('确认删除该记录吗?', '提示', {
          ...
          let para = {ids: array}
          let vm = this
          getApi('deleteResources', para, function (res) {
          ...
          if (valid) {
            this.$confirm('确认提交吗？', '提示', {}).then(() => {
              ...
              if (this.editFormContainer.permissionName == this.editForm.permissionName  && this.editFormContainer.describe == this.editForm.describe) {
                ...
                let vm = this
                getApi('modifyResources', modifyResourcesParams, function (res) {
