      ...
      getData () {
        ...
        getApi('searchRole', function (res) {
          if (res.data.code === 200) {
            ...
            self.roleid = res.data.data[0].roleOrder
          }
        })
      },
      menuList() {
        ...
        getApi('searchPermissions', params, function (res) {
          if (res.data.code === 200) {
            ...
            for (let i = 0; i < self.dataList.length; i++) {
              for (let j = 0; j < self.dataList[i].resources.length; j++) {
                if (self.dataList[i].resources[j].roleId != null && self.dataList[i].resources[j].roleId != 0) {
                  self.checkList.push(self.dataList[i].resources[j].permisstionId)
                }
              }
            }
          }
      ...
      addSubmit () {
        if (this.checkList.length >= 1) {
          this.$confirm('确认提交吗？', '提示', {}).then(() => {
            ...
            getApi('modifyPermisstion', para, function (res) {
