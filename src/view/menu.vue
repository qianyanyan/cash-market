  ...
  export default {
    ...
    methods: {
      ...
      searchFMenu (menuFParams) {
        let vm = this
        getApi('searchFMenu', menuFParams, function (res) {
          if (res.data.code === 200) {
            vm.fList = vm.fList.slice(0, 1).concat(res.data.data)
          }
        })
      },
      searchMenu (menuParams) {
        ...
        getApi('searchMenu', menuParams, function (res) {
          if (res.data.code === 200) {
      ...
      handleDel (index, row) {
        this.$confirm('确认删除该记录吗?', '提示', {
          ...
          let para = {ids: arr}
          let vm = this
          getApi('deleteMenu', para, function (res) {
            ...
            if (res.data.code == 200 && res.data.message.toLowerCase() == 'ok') {
              ...
              vm.searchMenu({
                ...
                menuStatus: vm.filters.value,
                startCreatetime: vm.startDate,
                endCreatetime: vm.endDate
              })
              vm.$store.commit('deleteMenu', row.id)
              vm.searchFMenu()
            } else {
              vm.$message({
          ...
          if (valid) {
            this.$confirm('确认提交吗？', '提示', {}).then(() => {
              ...
              if (this.editFormContainer.menuName == this.editForm.menuName && this.editFormContainer.menuUrl == this.editForm.menuUrl && this.editFormContainer.menuIcon == this.editForm.menuIcon && this.editFormContainer.menuOrder == this.editForm.menuOrder && this.editFormContainer.currentStatus == this.currentStatus) {
                ...
                let vm = this
                getApi('modifyMenu', menuParams, function (res) {
                  ...
                  if (res.data.code == 200 && res.data.message.toLowerCase() == 'ok') {
                    ...
                    vm.editForm.menuStatus = vm.editForm.menuStatus == '启用' ? 0 : 1
                    vm.$store.commit('changeMenuInfo', vm.editForm)
                    vm.searchMenu({
                      menuName: vm.filters.menuName,
                      menuStatus: vm.filters.value,
                      startCreatetime: vm.startDate,
        ...
        this.$refs.addForm.validate((valid) => {
          if (valid) {
            this.$confirm('确认提交吗？', '提示', {}).then(() => {
              ...
              let vm = this
              getApi('addMenu', addUserParams, function (res) {
