  ...
  <div>
    ...
    <el-dialog title="新增" v-model="addFormVisible" :close-on-click-modal="false">
      <el-form :model="addForm" label-width="80px" :rules="addFormRules" ref="addForm">
        ...
        <el-form-item label="状态">
          <el-switch on-text="启用" off-text="禁用" v-model="addForm.currentAddStatus"></el-switch>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        ...
        if (search) {
         ...
         let ddd = this.tableData.filter(function(item) {
            return Object.keys(item).some(function(key) {
              ...
              if (key == 'accounts' || key == 'createPeople' || key == 'roleName' || key == 'status') {
    ...
    methods: {
      ...
      handleDel (index, row) {
        this.$confirm('确认删除该记录吗?', '提示', {
          ...
          let para = { ids: arr }
          let vm = this
          getApi('removeUser', para, function (res) {
            ...
            if (res.data.code == 200 && res.data.message.toLowerCase() == 'ok') {
              ...
              vm.selectUser({
                status: vm.filters.value,
                startCreatetime: vm.startDate,
                endCreatetime: vm.endDate
              })
            } else {
              vm.$message({
          ...
          if (valid) {
            this.$confirm('确认提交吗？', '提示', {}).then(() => {
              ...
              if (this.editFormContainer.id == this.editForm.id && this.editFormContainer.accounts ==  this.editForm.accounts && this.editFormContainer.password == this.editForm.password && this.editFormContainer.currentStatus == this.currentStatus) {
                ...
                let vm = this
                getApi('modifyUser', userParams, function (res) {
                  ...
                  if (res.data.code == 200 && res.data.message.toLowerCase() == 'ok') {
                    ...
                    vm.selectUser({status: vm.filters.value, startCreatetime: vm.startDate, endCreatetime: vm.endDate})
                  } else {
                    vm.$message({
                      message: res.data.message,
                      type: 'error'
        ...
        this.$refs.addForm.validate((valid) => {
          if (valid) {
            this.$confirm('确认提交吗？', '提示', {}).then(() => {
              ...
              let addUserParams = {
                accounts: this.addForm.accounts,
                password: this.addForm.password,
                status: this.addForm.currentAddStatus ? 0 : 1
              }
              let vm = this
              getApi('addUser', addUserParams, function (res) {
    ...
    watch: {
      addFormVisible (val) {
        if (!val) {
          ...
          this.addForm.currentAddStatus = true
        }
      },
      searchData (val) {
